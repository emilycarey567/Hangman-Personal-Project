...
wrongLetters=0;%initalise wrongLetters to be 0
numberOfGuesses=6;%define number of guesses (6)


%Stage 6 while loop to determine how many guesses the user has left
while 1 %while user can still guess
...
if inputLengthCorrect>=1 %if the letter the user inputted occurs once or more in the selectedRandomWord 
    for i=1:inputLengthCorrect %for-loop, loops through the total amount correct
      if lengthOfSelectedRandomWord==correctLetters%if user guesses all letters correctly
          disp("you have won")%will be replaced with a function in Stage 7
        return
      else
...
if wrongLetters==6%if statement if the user has used up all of their guesses
    fprintf("%s\n",unknownLetters)%print remaining astriks
    disp("you lose")%will be replaced with function in Stage7
    break
else
